...
public class JdbcUserDao implements UserDao {
    ...
    public List<User> getUsers() {
        ...
        String sql = "SELECT user_id, password_hash, username, email, name, avatar, role FROM users";
        try {
    ...
    public User updateUserAvatar(User user) {
        ...
        try {
            int numberOfRows = 0;
            numberOfRows = jdbcTemplate.update(sql, user.getAvatar(), user.getId());
    ...
    public User updateUser(User user, int currentUserId) {
        ...
        if(user.getNewPassword() != null){
            ...
            sql = "UPDATE users SET email = ?, password_hash = ?, name = ? " +
                    "WHERE user_id = ?";
        }
        else{
        ...
        try{
            ...
            if(!password_hash.equals("")) {
                numberOfRows = jdbcTemplate.update(sql, user.getEmail(), password_hash
                        , user.getName(), currentUserId);
            }
            else{
                numberOfRows = jdbcTemplate.update(sql, user.getEmail(), user.getName(), currentUserId);
            }
            if(numberOfRows == 0){
                throw new DaoException("No number of rows affected");
            }
